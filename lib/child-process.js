...
class ChildProcess {
  ...
  async exec(execPath, command) {
    ...
    if (process.pkg) {
      ...
      const tmpExecPath = path.join(tmp, execName);
      logger.debug(`exec path copying... tmp folder: ${tmp}`);

      // Read all files from the bundled directory and copy them
      const files = fs.readdirSync(execFolder);
      for (const file of files) {
        const srcFile = path.join(execFolder, file);
        const destFile = path.join(tmp, file);

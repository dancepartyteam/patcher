...
class Utils {
  ...
  async getLatestVersion() {
    try {
      ...
      if (res.status !== 200) throw new Error(`GitHub API responded with status ${res.status}`);
      
      const latestVersion = res.data.tag_name.replace(/^v/, ''); // remove leading 'v' if present
  ...
  getPatchedFilePath(inputPath, format, game, gameId, platform = "wii") {
    ...
    const name = `DanceParty Legacy (${game.version}) ${gameId} (${game[platform]["ids"][gameId].r}).${format.toLowerCase()}`;

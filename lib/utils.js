...
class Utils {
  ...
  async getLatestVersion() {
    try {
      const url = 'https://api.github.com/repos/dancepartyteam/patcher/releases/latest';
      ...
      const latestVersion = data.tag_name.replace(/^v/, ''); // remove leading 'v' if present
  ...
  /**
   * @deprecated This is no longer used since CLI now requires user to provide the info about the game.
   * @param {*} inputPath 
   * @returns 
   */
  async detectFormat(inputPath) {
    ...
    // BIN files (PS3's EBOOT.BIN)
      ...
      logger.warn(`BIN files are not supported, please provide an ISO, WBFS or a DOL file.`);
      process.exit();
    }
  ...
  getPatchedFilePath(inputPath, format, game, gameId, platform = "wii") {
    const originalDir = path.dirname(inputPath);
    const name = `DanceParty Legacy (JD${game.version}) ${gameId} (${game[platform]["ids"][gameId].r}).${format.toLowerCase()}`;

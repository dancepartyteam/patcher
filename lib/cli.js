...
If no arguments are provided, the patcher will run in interactive mode.`)
    ...
    .option("list", {
      alias: "l",
      type: "string",
      description: "List available games for a platform (wii)"
    })
    .example([
  ...
  if (hasCliArgs) {
    ...
    if (argv.id && !argv.platform && !argv.gameVersion) {
      ...
      if (!result) {
        ...
        console.error("Use --list wii to see available game IDs\n");
        process.exit(1);
      }

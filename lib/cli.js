...
Supports Just Dance 2 to 2020 including the spin-offs for Wii and PS3.
...
module.exports = async () => {
  ...
  if (argv.list) {
    ...
    if (games.length === 0) {
      ...
      games.forEach(g => {
        ...
        if (platform === "WII" && g.wii.ids) {
          const ids = Object.keys(g.wii.ids).map(id => `${id} (${g.wii.ids[id].r})`).join(", ");
          console.log(`    IDs: ${ids}`);
        }
      });
      console.log();
  ...
  if (hasCliArgs) {
    // Validate that file is always provided
    if (!argv.file) {
    ...
    let platform, selectedGame, gameId, region;
    ...
    if (argv.id && !argv.platform && !argv.gameVersion) {
      ...
      console.log(`  Game ID: ${gameId} (${region})`);
    ...
    // Case 2: Platform and game version provided
      ...
      if (platform === "WII") {
        if (!argv.id) {
          ...
          const availableIds = Object.keys(selectedGame.wii.ids).map(id => `${id} (${selectedGame.wii.ids[id].r})`).join(", ");
        ...
        if (!selectedGame.wii.ids[gameId]) {
          const availableIds = Object.keys(selectedGame.wii.ids).map(id => `${id} (${selectedGame.wii.ids[id].r})`).join(", ");
        ...
        region = selectedGame.wii.ids[gameId].r;
      }

      console.log("\n✓ Using CLI arguments:");
      console.log(`  Platform: ${platform}`);
      console.log(`  Game: ${selectedGame.name} (v${argv.gameVersion})`);
      if (gameId) console.log(`  Game ID: ${gameId} (${region})`);
      console.log(`  Input File: ${argv.file}`);
      console.log(`  Format: ${format.type} (${format.filename})\n`);
    }
      ...
      const gameIdPrompt = await inquirer.prompt([
        {
          ...
          choices: gameIds.map(id => {
            const regionValue = selectedGame.wii.ids[id].r;
            return {
              name: `${id} (${regionValue})`,
              value: id
            };

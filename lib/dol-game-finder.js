...
module.exports = (inputPath) => {
  ...
  for (const game of config.WII.GAMES) {
    
    // LyN games have a unique string we can search for.
    if (game.isLyN && game.string) {
      if (mainDol.includes(game.string)) {
        jdVersion = game.version;
        break;
      }
    }
    else {
